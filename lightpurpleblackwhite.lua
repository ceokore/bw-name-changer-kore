...
local Colors = {
    ...
    Color3.fromRGB(200,170,255),  -- light purple
    Color3.fromRGB(0,0,0)         -- back to black
}

local function GetBWColor(dt)
    time += dt * FadeSpeed

    local count = #Colors - 1
    local t = (math.sin(time) + 1) / 2 * count

    local index = math.floor(t) + 1
    local alpha = t % 1

    return Colors[index]:Lerp(Colors[index + 1], alpha)
end
